...
/**
 * 생성된 HTML을 실제 puppeteer viewport(mobile/tablet/desktop)에서 렌더링해
 * 반응형·레이아웃·콘텐츠 밀도·3D 통합·한글 깨짐 등의 결함을 검출한다.
 *
 * generate / expand 라우트 양쪽에서 공유한다.
 */
export async function auditResponsiveHtml(
  browser: Browser,
  html: string,
  options: { requireLogo?: boolean } = {},
): Promise<string[]> {
  const viewports = [
...
/**
 * auditResponsiveHtml가 검출한 이슈를 refineUI에 전달할 한국어 수정 지침으로 변환한다.
 */
export function buildResponsiveRepairMessage(responsiveIssues: string[], repairAttempt: number): string {
  return `생성된 HTML이 실제 viewport 반응형 검사를 통과하지 못했습니다. 이번 수정은 ${repairAttempt}/2번째 반응형 repair입니다.
