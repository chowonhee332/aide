...
const DOMAIN_CONTEXT: Record<AppDomain, DomainContext> = {
  finance: {
    a: '토스·뱅크샐러드처럼: 잔액·거래내역 숫자가 화면을 지배하고, 프라이머리 버튼 1개에만 accent 사용. 테이블형 리스트, 날짜/금액 정렬 강조.',
    b: '로빈후드·미래에셋처럼: 수익률 숫자 72px/900w로 히어로 카드 가득 채우기. 실시간 차트가 히어로 카드 하단에 바로 이어짐. 상승/하락 시맨틱 강조.',
    c: '카카오페이·N26처럼: 돈보다 "목표/꿈" 중심 감성 카피 + 일러스트/3D 에셋. 저금통·목표 카드에 그라데이션, 달성률은 링 애니메이션으로 표현.',
  },
  commerce: {
    ...
    c: '29CM·오늘의집처럼: 무드 있는 라이프스타일 이미지 우선, 제품명보다 스토리 카피. 넉넉한 여백, 에디토리얼 레이아웃.',
  },
  health: {
    a: '삼성헬스·애플 피트니스처럼: 걸음수·칼로리·운동 시간 숫자 크게, 링/바 차트 중심. 목표 달성 시 accent 1개에만 사용. 주간 통계 라인 차트.',
    b: '스트라바·나이키런처럼: 오늘의 운동 기록 72px/900w로 히어로. 지도 궤적 또는 페이스 차트가 히어로 카드 배경. 퍼스널베스트 뱃지 강조.',
    c: '캄·헤드스페이스처럼: 자연/힐링 이미지가 상단 전폭, 감성 카피("오늘도 잘 하고 있어요"). 부드러운 그라데이션 오버레이, 숫자보다 감정 중심.',
  },
  food: {
    ...
    c: '마켓컬리·오이식스처럼: 신선한 식재료 모습·식탁 무드샷 에디토리얼. 생산자 스토리 카드, 넉넉한 여백.',
  },
  productivity: {
    a: '노션·리니어처럼: 작업 리스트 조밀한 표 형태, 상태·우선순위·담당자 컬럼 명확. 사이드바 네비게이션, 모든 요소 grid-aligned. accent는 상태 뱃지에만.',
    b: '지라·아사나 대시보드처럼: 이번 주 완료/잔여 태스크 숫자 72px 히어로. 번다운 차트가 바로 아래, 팀 진척률 게이지 바. 진척 상태 시맨틱 강조.',
    c: '비헨스·피그마처럼: 프로젝트 썸네일 대형 그리드, 커버 이미지 중심. 크리에이티브 도구 특유의 비주얼 임팩트 레이아웃.',
  },
  social: {
    ...
    c: '비리얼·폴라로이드처럼: 자연스러운 일상 사진 중심, 날짜 타임스탬프 손글씨체, 감성적인 필터 효과.',
  },
  travel: {
    ...
    c: '에어비앤비 홈·부킹닷컴처럼: 여행지 무드 사진 에디토리얼, "어디든 갈 수 있어요" 감성 카피. 그라데이션 오버레이, 여백 넉넉.',
  },
  education: {
    ...
    c: '마스터클래스처럼: 강사 시네마틱 사진 전폭 히어로, 감성 카피 "배움의 격을 높이다". 에디토리얼 강사 소개, 고급스러운 레이아웃.',
  },
  entertainment: {
    a: '왓챠·웨이브처럼: 콘텐츠 썸네일 조밀한 가로 스크롤 리스트, 장르·평점·러닝타임 small text. 다크 계열 표면, 보조 텍스트 약하게.',
    b: '넷플릭스처럼: 대형 히어로 배너가 화면 상단 2/3 차지, 제목 72px white bold, 재생/찜 버튼 히어로 내. 배경 영상/이미지 풀블리드.',
    c: '스포티파이처럼: 콘텐츠 이미지 기반 그라데이션 배경, 무드 플레이리스트 에디토리얼. 아티스트 감성 사진 + 큐레이션 카피.',
  },
  business: {
    a: '세일즈포스·허브스팟처럼: 지표 테이블 + 바·라인 차트 대시보드. 숫자는 모노스페이스, 상태 뱃지 시맨틱 코드. 사이드바 네비, 공백 최소화.',
    b: '그로우·태블로처럼: 핵심 KPI 3개가 상단 히어로 영역 full-width 차지, 72px/900w. 메인 차트(라인/바)가 바로 아래 대형으로. 전환율·매출 숫자 강조.',
    c: '노션 팀 홈·컨플루언스처럼: 팀 무드 이미지/일러스트, 감성 환영 배너. 팀원 아바타, 주간 하이라이트 카드. 친근한 워크플레이스 분위기.',
  },
  other: {
    a: '클린하고 정보 중심인 앱처럼: 흰 배경, 핵심 데이터를 명확한 계층 구조로. accent는 액션 요소에만, 그림자 카드 그리드.',
    b: '임팩트 있는 앱처럼: 핵심 수치/메시지가 상단 히어로를 가득 채움. 72px/900w 타이포, primary 히어로 카드, 아래 서포팅 카드.',
    c: '감성 있는 브랜드처럼: 무드 이미지·일러스트·3D 에셋이 첫인상. 그라데이션 히어로, 감성 카피, 넉넉한 여백.',
  },
};
...
- 컬러: 디자인 시스템 primary 토큰을 히어로·버튼·ring/badge에 적극 사용 (임의 색상 지정 절대 금지)
