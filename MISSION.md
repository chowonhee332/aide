...
- **검증**: 🔴 RED — `verify_sales_input_history` 키보드 안내 계약 불일치(`page.tsx` 미변경, 이번 세션 밖). **이제 마스킹 안 됨**: `npm test`가 `scripts/run-all-tests.mjs` 러너로 바뀌어 16개 전부 실행하고 실패만 집계(`ac12bbd`). 나머지 15개 + lint + build 통과.
...
- **Brain 직접(2026-09-08) — Astryx 컴포넌트 전면 교체(버튼 단계 완료)**: 🟢 커밋 `acc767c`·`9c3a14b`·`876c1a9`·`7e49020`·`3df9417`·`ef37214`·`e0be466`~`0471b0f`(1~5차)·`b57c672`. **범위**(사용자 승인): 손수 만든 버튼·입력·칩·카드·패널·배너 → Astryx 컴포넌트, 순수 레이아웃 div는 유지. **page.tsx 홈 완료**: 모달→`Dialog`, 브리프/소스/브랜드/design.md 패널의 버튼·입력·칩 전부 `Button`/`IconButton`/`TextInput`/`TextArea`/`Badge`. **BuilderView 완료**: 라이브러리 탭바→`SegmentedControl`, 속성/AI편집/프레임추가 버튼 교체. **StudioView 완료**(HIGH-RISK, 배치 1~5): IR 재정렬, 챗 리파인(입력창→`TextArea`), 히어로/서브스크린/플랫폼 토글→`SegmentedControl`, 프로토타입 툴바, Creon, 아이콘 피커, TweaksModal, `Toggle`→Astryx `Switch`, `AlignField`, 질문 옵션 칩. `check-studio-contract.mjs` 앱/웹 정규식을 SegmentedControl 형태로 갱신. **잔여 ~19개 `<button>`은 의도적 예외**: 프리뷰 그리드(AGENTS #6), 롤백 스위치(AGENTS #8), 로고/뒤로 내비, 공유·줌 드롭다운 메뉴 행, 추출 색상 스와치, 아이콘 그리드 타일, "메인 구조" 질문 카드(링크·메뉴·콘텐츠 타일 성격). **유지**: 네이티브 `<input type=color>`, 검색결과 이미지 타일. 전 커밋 `lint` 0 err · `build` ✓ · `npm test` 18/20(기존 RED 2개). **후속(`b204088`)**: 홈 히어로에 남아 있던 raw `<button>` 2개도 교체 — "상세 입력" 디스클로저→Astryx `Collapsible`(자체 셰브런), "design.md 없으신가요?" 밑줄 링크→Astryx `Link`. `ChevronDown` import 제거.
...
- **Brain 직접(2026-09-08) — 컨트롤 크기 밀도 상속**: 🟢 커밋 `1730478`(→회수)·`02b57a4`·`12d0300`·`d990868`(미푸시). `AideDensityProvider`가 이미 Astryx `<SizeProvider>`(comfortable→lg)를 앱 루트에서 내려주는데(`Button`=`useSize(sizeProp,'md')`), 개별 `size="sm"` 하드코딩이 이를 무력화해 **한 행에서 42px/30px가 섞였다**. 방향(사용자 결정): "밀도가 전부 제어" — 주요 컨트롤은 `size` 제거해 상속, "이웃보다 의도적으로 작은" 것만 예외. 고친 곳: 홈 브리프 액션 행+패널 버튼 14곳, Playground 58px 툴바 3버튼, Studio Step3 56px 헤더 3버튼. 검증: 홈 행 comfortable 42 / compact 34로 함께 스케일. **유지**: 칩·썸네일·스와치 X 제거 버튼, Studio `h-9`(36px) 고정 툴바(전부 sm 일관 + 변경 시 오버플로), 텍스트/직접만든 크롬 옆 단독 컨트롤. **선행 발견 버그**(칩 `task_7abf68fd`): `AideDensityProvider`가 `useState` 초기화에서 localStorage 읽음 → 비기본 밀도 선택 시 하이드레이션 mismatch(`page.tsx` `f4097e7` 딥링크와 같은 클래스).
